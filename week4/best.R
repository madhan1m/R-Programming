setwd("D:\\Excercises\\ML_Projects\\R\\week4")

list.files()

# helper function for getting the hospital name

besthospital <- function(data, col_num, state) {
...
best <- function(state, outcome) {
  ...
  if (!state %in% data$State) {
    ...
    if(outcome == "heart attack") {
      ...
      hosp_name <- besthospital(data, 11, state)
      
    } else if(outcome == "heart failure") {
      
      hosp_name <- besthospital(data, 17, state)
      
    } else {
      
      hosp_name <- besthospital(data, 23, state)
